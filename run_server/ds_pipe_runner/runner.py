...
base_file_name = f"pl-{args.percent_labelled}_ns-{args.number_of_samples}_q-{args.quality_measure}"
...
if args.algorithm == "knn_ldp": 
    ...
    f = open(f"{user_folder}/alg-knnldp_{base_file_name}_n-{args.n_neighbors}.csv", "a+")

elif args.algorithm == "lp": 
    if args.kernel == "rbf":
        algorithm = LabelPropagation(gamma=args.gamma)
        f = open(f"{user_folder}/alg-lp_{base_file_name}_g-{args.gamma}.csv", "a+")
    else: 
        algorithm = LabelPropagation(n_neighbors=args.n_neighbors)
        f = open(f"{user_folder}/alg-lp_{base_file_name}_n-{args.n_neighbors}.csv", "a+")

elif args.algorithm == "ls": 
    if args.kernel == "rbf":
        algorithm = LabelSpreading(gamma=args.gamma, alpha=args.alpha)
        f = open(f"{user_folder}/alg-ls_{base_file_name}_g-{args.gamma}_a-{args.alpha}.csv", "a+")
    else: 
        algorithm = LabelSpreading(n_neighbors=args.n_neighbors)
        f = open(f"{user_folder}/alg-ls_{base_file_name}_n-{args.n_neighbors}.csv", "a+")
else: 
